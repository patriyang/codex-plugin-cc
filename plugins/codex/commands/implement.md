...
- If a dispatch exits non-zero or returns empty or malformed output, treat it as `BLOCKED` (step 3) rather than assuming the step succeeded. A failed dispatch may already have applied edits: inspect the payload's `touchedFiles` and `git status` / `git diff` in `WORKTREE_ROOT` before re-dispatching, and resume the existing thread with `--resume-id "${IMPLEMENTER_THREAD_ID}"` rather than dispatching a fresh implementer on top of applied work.
