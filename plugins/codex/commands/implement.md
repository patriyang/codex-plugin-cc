...
argument-hint: "[--sequential|--single-shot] [--background|--wait] [--model <model|spark|astra>] [--effort <none|minimal|low|medium|high|xhigh|max|ultra>] [plan or path to plan]"
...
- If enqueueing or result retrieval exits non-zero, the job ends as `failed` or `cancelled`, or any returned JSON or report is empty or malformed, treat the step as `BLOCKED` (step 3) rather than assuming it succeeded. Recover according to which role failed:
  - **A failed implementer dispatch may already have applied edits** even though its report never arrived — `.storedJob.result.rawOutput` is empty in that case, with the aborted turn's partial text under `.storedJob.result.partialOutput` and the reason under `.storedJob.result.failureMessage`. Inspect `.storedJob.result.touchedFiles`, which covers native file changes and `apply_patch` shell edits, while `git status` / `git diff` in `WORKTREE_ROOT` remains authoritative because other shell-applied edits are not attributable. Then set `const threadIdArg = shellEscape(IMPLEMENTER_THREAD_ID)` and resume that same thread with `--resume-id ${threadIdArg}`. Never dispatch a fresh implementer on top of applied work.
