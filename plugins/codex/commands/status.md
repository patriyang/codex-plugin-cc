...
- Claude cannot invoke this slash command (`disable-model-invocation: true`); a controller waiting on behalf of another flow must call `codex-companion.mjs status ... --wait` directly in a foreground `Bash` call. Set the `Bash` tool's timeout comfortably larger than `--timeout-ms`; if the JSON says `waitTimedOut: true`, issue another bounded foreground wait for the same job ID. Once the payload is terminal, read the persisted output with `codex-companion.mjs result <job-id> --json` and continue the work it gated.
