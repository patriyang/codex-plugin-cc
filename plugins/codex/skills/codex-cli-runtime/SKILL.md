...
- Leave model unset by default for task requests; the runtime defaults to `gpt-6-astra`. Review commands resolve their own defaults: `/codex:deep-review` uses `gpt-6-astra` with `high` effort, and each review's output names the model it ran under — plus the effort, except for the native `/codex:review`, which sends none. Add `--model` only when the user explicitly asks for one.
