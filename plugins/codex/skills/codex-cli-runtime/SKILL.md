...
- `--effort`: accepted values are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. Support is per model: `max` and `ultra` require a GPT-5.6 model (`ultra` only `gpt-5.6-sol` / `gpt-5.6-terra`); older models top out at `xhigh`, and an unsupported pair is rejected by Codex, not by the plugin.
