...
- Leave model unset by default for task requests; the runtime defaults to `gpt-5.5`. Review commands resolve their own defaults: `/codex:deep-review` uses `gpt-5.6-sol` with `medium` effort, and each review's output states which model and effort ran. Add `--model` only when the user explicitly asks for one.
