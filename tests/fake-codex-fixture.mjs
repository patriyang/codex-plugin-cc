  ...
  try {
    switch (message.method) {
      ...
      case "thread/resume": {
        ...
        const thread = ensureThread(state, message.params.threadId);
        thread.updatedAt = now();
        state.lastThreadResume = { threadId: message.params.threadId, model: message.params.model ?? null, sandbox: message.params.sandbox ?? null, approvalPolicy: message.params.approvalPolicy ?? null, config: message.params.config };
        saveState(state);
        send({ id: message.id, result: { thread: buildThread(thread), model: message.params.model || "gpt-5.4", modelProvider: "openai", serviceTier: null, cwd: thread.cwd, approvalPolicy: "never", sandbox: { type: "readOnly", access: { type: "fullAccess" }, networkAccess: false }, reasoningEffort: null } });
        break;
      }
